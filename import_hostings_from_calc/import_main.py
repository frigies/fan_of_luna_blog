...
# В основном скрипте после импорта добавьте:
def clean_prices_in_db():
    """Очищает цены в базе данных после импорта"""
    try:
        ...
        # Сначала посчитаем проблемные записи
        cur.execute("""
            SELECT COUNT(*) FROM hosting 
            WHERE min_price_in_dollars IS NOT NULL 
            AND (min_price_in_dollars < 0 OR min_price_in_dollars > 10000);
        """)
        problem_count = cur.fetchone()[0]
        print(f"Найдено проблемных цен: {problem_count}")
        
        # Обновляем проблемные цены в NULL
        cur.execute("""
            UPDATE hosting 
            SET min_price_in_dollars = NULL 
            WHERE min_price_in_dollars IS NOT NULL 
            AND (min_price_in_dollars < 0 OR min_price_in_dollars > 10000);
        """)
        ...
        print(f"Ошибка при очистке цен: {e}")
...
def parse_price(price_cell):
    ...
    # Если пустая строка или содержит только нечисловые символы
    ...
    # Проверяем, является ли числом после очистки
    if not cleaned_price:
        return None
    
    # Проверяем, состоит ли только из цифр и точки
    if re.match(r'^[0-9]*\.?[0-9]+$', cleaned_price):
        try:
            return float(cleaned_price)
        except (ValueError, TypeError):
            print(f"Ошибка преобразования цены: '{price_str}' -> '{cleaned_price}'")
            return None
    else:
        print(f"Цена не является числом: '{price_str}' -> '{cleaned_price}'")
        return None


# Функция для определения favorite по названию или другим признакам
def is_favorite(hosting_name, status):
    """
    Определяем, является ли хостинг избранным
    Можно настроить логику по названию, статусу и т.д.
    """
    if not hosting_name:
        return False
    ...
    name_str = str(hosting_name).lower()
...
# Определяем favorite
print("\nОпределение избранных хостингов...")
df['favorite'] = df.apply(
    lambda row: is_favorite(row['hosting_name'], row.get('Статус')), 
    axis=1
)
...
print(f"Избранных хостингов: {df['favorite'].sum()}")

# # Показываем примеры цен для отладки
# print("\nПримеры обработки цен:")
# price_samples = df[['Минимальная цена', 'min_price_in_dollars']].head(10)
# for idx, row in price_samples.iterrows():
#     print(f"  '{row['Минимальная цена']}' -> {row['min_price_in_dollars']}")
        ...
        try:
            ...
            cur.execute("""
                INSERT INTO hosting (
                    ...
                    disadvantages, hosting_location, servers_location, 
                    min_price_in_dollars, favorite
            ...
            # Присваиваем остальные категории (кроме "Избранное")
            categories_to_assign = [1]  # Например, только "За крипту"
